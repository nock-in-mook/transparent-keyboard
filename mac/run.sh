...
# --install: ローカルにコピーしてログイン時自動起動を設定
# 引数なし: 直接起動

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SRC="$SCRIPT_DIR/transparent_keyboard_mac.py"
DEST="$HOME/.local/bin/transparent_keyboard_mac.py"
...
<dict>
    ...
    <array>
        ...
        <string>DEST_PLACEHOLDER</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
...
PLIST_EOF
    # プレースホルダーを実際のパスに置換
    sed -i '' "s|DEST_PLACEHOLDER|$DEST|" "$PLIST"

    # LaunchAgent を登録
    launchctl unload "$PLIST" 2>/dev/null
    launchctl load "$PLIST"
    echo "LaunchAgent 登録完了（ログイン時に自動起動します）"
    echo "今すぐ起動します..."
