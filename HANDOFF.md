...
## 今回の変更（2026-04-17）
### Mac版: キー送信を CGEventPost → osascript に切替（永続バグ修正）
- 長年の「テキスト送信ボタンが反応しない、再起動するまで戻らない」症状を解消
- 原因: macOS の TCC キャッシュ問題で、Python（`com.apple.python3`）への CGEventPost 権限が不安定だった
- 対策: `mac/transparent_keyboard_mac.py:83-103` の `send_key()` / `type_text()` を `osascript -e 'tell System Events to ...'` 経由に置き換え
- スクショボタン（`screencapture` を subprocess 起動）と同じく別プロセス方式になり、Python 自身のアクセシビリティ権限が不要に
- 詳細は DEVLOG.md 参照
